...
func Write(out io.Writer, exec *testjson.Execution, cfg Config) (JUnitTestSuites, error) {
	...
	err := write(out, tests)
	if err != nil {
		err = fmt.Errorf("failed to write JUnit XML %w", err)
	}
	return tests, err
}
...
func generate(exec *testjson.Execution, cfg Config) JUnitTestSuites {
	...
	for _, pkgname := range exec.Packages() {
		...
		var c []JUnitTestCase
		for _, ca := range packageTestCases(pkg, cfg.FormatTestCaseClassname) {
			c = append(c, *ca)
		}
...
func goVersion() string {
	...
	out, err := cmd.Output()
	if err != nil {
		fmt.Fprint(os.Stderr, "failed to lookup go version for junit xml")
		return "unknown"
	}
...
func packageTestCases(pkg *testjson.Package, formatClassname FormatFunc) []*JUnitTestCase {
	...
	for _, tc := range pkg.Skipped {
		jtc := newJUnitTestCase(tc, formatClassname)
		jtc.SkipMessage = &JUnitSkipMessage{Message: pkg.Output(tc.ID)}
		cases = append(cases, &jtc)
	}
